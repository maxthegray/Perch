...
- **Tuck the shelf away.** Rest the pointer against the edge Perch is on to slide it away with its items intact. Move away and touch the same edge again to bring it back. Toggle this in Behavior settings.
- **Bring the shelf to you.** Turn on Shake pointer to show and a shake brings Perch to the cursor. Move a floating shelf with its handle or Command-drag and optionally lock it in place. Other enabled edges offer themselves while you drag; you can also let an open shelf move when you touch another edge.
...
**General** has launch at login, the installed version, and update checks.

**Shelf** controls the card's style, layout, size, labels, shadow, available screen edges, and Dock snapping.

**Behavior** controls when Perch appears, how the shelf moves, whether it can switch edges when you touch them, and whether an empty shelf stays open.

**Files** follows a file from drag in to drag out. It controls where dropped files are stored, recent arrivals, whether transforms keep their originals, and whether dragged-out items remain in Perch.
